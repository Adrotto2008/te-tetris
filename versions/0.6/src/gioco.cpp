...
void Gioco::listaStanze(){
    ...
    short i = 5;
    COORD posizione;
    short stanze_esistenti = 2, stanze_massime = 10;
    std::array<std::string, 9> nome_stanza;
    ...
    cursore_manuale(5, 2);
    printf("stanze : (%hd/%hd) ", stanze_esistenti, stanze_massime);

    for(short j = 0; j < 9; j++, i += 5){

        cursore_manuale(5, i);
        printf("  %-10s", nome_stanza[j]);
